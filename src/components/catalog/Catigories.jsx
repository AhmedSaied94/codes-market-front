...
const Catigories = ({catigories}) => {
    ...
    const onOpenChange = keys => {
      ...
      console.log(k)
      if (k){
        axiosFetchInstance(`/items/catigory=${k[k.length-1]}/`)
        .then(res => {
    ...
    const handleClick = e => {
        console.log(e.key)
        axiosFetchInstance(`/items/sub_catigory=${e.key[e.key.length-1]}/`)
  ...
  return (
    ...
    >
        {catigories.map(cat => {
            return (
                <SubMenu  className='sub-tab' key={`cat ${cat.id}`} icon={<AppstoreOutlined />} title={cat.name}>
