...
const ItemDetails = (props) => {
    ...
    React.useEffect(()=> {
        const checkLike = item.likes && item.likes.find(like => like.user === authedUser.id)
        const checkwish = authedUser.wishlist_items && authedUser.wishlist_items.find(i => i.id === item.id)
        if (item.seller === authedUser.username) {
            setOwnerState(true)
        }
        else{
    ...
    const handleLike = operation => {
        axiosFetchInstance.get(`/handle-likes/${item.id}/${operation}/`)
        .then(res => {
    ...
    const handleWishlist = operation => {
        axiosFetchInstance.get(`/handle-wishlist/${item.id}/${operation}/`)
        .then(res => {
            message.success(res.data.success)
            wishlistState === 'wish' ?
  ...
  return (
    ...
    <div>
        ...
        <div style={{display:'flex', justifyContent:'space-evenly', flexWrap:'wrap', backgroundColor:'#fff', border:'1px solid #f0f0f0', padding:'1rem 0'}}>
            <Button onClick={() => window.open(item.demo_url, '_blank')} style={{margin:'0.25rem 0'}} type='primary'>Live Demo</Button>
            {
            ownerState  ?
                <Button disabled style={{margin:'0.25rem 0'}} type='primary'>Likes {item.likes.length}</Button>
            :
            <>
            <Button style={{margin:'0.25rem 0'}}
            onClick={likeState === 'like'? ()=> handleLike('like') : ()=> handleLike('unlike')} 
             type='primary'>{likeState === 'like'? 'Like' : 'Unlike'}</Button> 

            <Button  style={{margin:'0.25rem 0'}} 
            onClick={wishlistState==='wish'? ()=> handleWishlist('add'): ()=>handleWishlist('remove')} 
            type='primary'>{wishlistState==='wish'? 'Add to Wishlist': 'Remove from Wishlist'}</Button>
            </>
            }
        </div>
    </div>
